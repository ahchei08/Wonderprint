...
void GCode::_do_export(Print& print, GCodeOutputStream &file, ThumbnailsGeneratorCallback thumbnail_cb)
{
    ...
    if(!has_BTT_thumbnail){   
        ...
        // ahchei file.write_format("; generated by %s on %s\n", Slic3r::header_slic3r_generated().c_str(), Slic3r::Utils::local_timestamp().c_str());
        file.write_format("; generated by %s on %s\n", "OrcaSlicer 2.3.1", Slic3r::Utils::local_timestamp().c_str());
        if (is_bbl_printers)
    ...
    std::vector<float> flush_mult(cast<float>( m_config.flush_multiplier.values));
    ...
    for (int i = 0; i < flush_matrix.size(); i++) {
        ...
        snprintf(buffer, sizeof(buffer), "%d", int(flush_matrix[i] * flush_mult[0] + EPSILON));
        // else
